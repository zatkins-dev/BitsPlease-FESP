...
class headsUpDisplay():
    ...
    def updateHUD(self, x, y, pDeg, vMag, vDeg, aMag, aDeg, components):
        ...
        graph.drawText((192,50), "Nose Degree: " + str("{:10.4f}".format(self._positionDegree)) + " degrees", 20, (255,0,0), "Times New Roman")
        graph.drawText((210,70), "Velocity Magnitude: " + str("{:10.4f}".format(self._velocityMag)) + " m/s", 20, (255,0,0), "Times New Roman")
        graph.drawText((217,90), "Velocity Degree: " + str("{:10.4f}".format(self._velocityDegree)) + " degrees", 20, (255,0,0), "Times New Roman")
        graph.drawText((247,110), "Acceleration Magnitude: " + str("{:10.4f}".format(self._accelerationMag)) + " m/s^2", 20, (255,0,0), "Times New Roman")
        graph.drawText((241,130), "Acceleration Degree: " + str("{:10.4f}".format(self._accelerationDegree)) + " degrees", 20, (255,0,0), "Times New Roman")

        numThruster = 0
        for thruster in self.thrusters:
            numThruster = numThruster + 1
            graph.drawText((312,130 + numThruster*20), "Thruster Module " + str(numThruster) + " Fuel Remaining: " + str("{:10.0f}".format(thruster.fuel)) + " Liters", 20, (255,0,0), "Times New Roman")

        numSAS = 0
        for sas in self.SASmodules:
            numSAS = numSAS + 1
            graph.drawText((282,130 + numThruster*20 + numSAS*20), "SAS Module " + str(numSAS) + " Fuel Remaining: " + str("{:10.0f}".format(sas.fuel)) + " Liters", 20, (255,0,0), "Times New Roman")
